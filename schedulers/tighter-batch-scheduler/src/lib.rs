...
use schedulers::tighter_batch::{TighterBatchConfig, derive_costs, tighter_batch_score};
...
#[derive(Debug)]
pub struct TighterBatchSchedulerArgs {
    ...
    pub scoring: TighterBatchConfig,
    pub runtime: RuntimeConfig,
}

pub struct TighterBatchScheduler {
...
impl TighterBatchScheduler {
    #[must_use]
    pub fn new(
        shutdown: CancellationToken,
        events: Option<EventEmitter>,
        args: TighterBatchSchedulerArgs,
    ) -> (Self, JoinHandle<()>) {
...
mod tests {
    ...
    fn test_scheduler() -> (TighterBatchScheduler, crossbeam_channel::Sender<JitoUpdate>) {
        let (jito_tx, jito_rx) = crossbeam_channel::bounded(1024);
    ...
    fn execute_schedules_by_priority() {
        ...
        let tx_low = noop_with_budget(&payer_low, 100_000, 100);
        let tx_high = noop_with_budget(&payer_high, 10_000, 200);
    ...
    fn bundle_scheduled_over_tx_when_higher_priority() {
        ...
        let tx = noop_with_budget(&payer_tx, 100_000, 100);
        bridge.queue_tpu(&tx);
        ...
        let bundle_tx = noop_with_budget(&payer_bundle, 10_000, 500);
        jito_tx
            .send(JitoUpdate::Bundle(vec![serialize_tx(&bundle_tx)]))
            .unwrap();
        ...
        assert_eq!(scheduler.checked_tx.len(), 0, "no more transactions left");
    }
    ...
    fn checked_capacity_eviction() {
        ...
        for (i, payer) in payers.iter().enumerate() {
            let fee = ((i + 1) as u64) * 100_000;
            let tx = noop_with_budget(payer, 25_000, fee);
            bridge.queue_tpu(&tx);
        }
        bridge.queue_progress(MOCK_PROGRESS);
        scheduler.poll(&mut bridge);
        ...
        let new_tx = noop_with_budget(&new_payer, 25_000, 10_000_000);
    ...
    fn bundle_accepted_when_all_txs_above_min_score() {
        ...
        let payer_a = Keypair::new();
        let payer_b = Keypair::new();
        let tx_a = noop_with_budget(&payer_a, 10_000, 50_000);
