...
pub const ALLOCATOR_SIZE: usize = 512 * 1024 * 1024;
const TPU_TO_PACK_CAPACITY: usize = 131072;
const PACK_TO_WORKER_CAPACITY: usize = 131072;
const WORKER_TO_PACK_CAPACITY: usize = 131072;

pub struct BenchBridge<M>
where
    M: Copy,
{
...
impl<M> Deref for BenchBridge<M>
where
    M: Copy,
{
...
impl<M> DerefMut for BenchBridge<M>
where
    M: Copy,
{
    ...
    M: Copy,
{
    ...
    pub fn new(worker_count: usize, worker_req_cap: usize) -> Self {
        ...
        let logon = ClientLogon {
            worker_count,
            allocator_size: ALLOCATOR_SIZE, // Use harmonic's allocator size!
            allocator_handles: 1,
    ...
    pub fn sync_producer_queues(&mut self) {
        self.agave.progress_tracker.sync();
        self.agave.tpu_to_pack.producer.sync();
    }
    ...
    pub fn queue_check_response(
        ...
        if let Some(keys) = keys {
            ...
            self.pending_frees.push((worker_idx, offset));
    ...
    fn build_single_tx_batch_region(
        ...
        self.pending_frees.push((worker_idx, batch_offset));
    ...
    pub fn free_pending_allocations(&mut self) {
        for (worker_idx, offset) in self.pending_frees.drain(..) {
            let worker = &mut self.agave.workers[worker_idx];
            unsafe {
                worker.allocator.free_offset(offset);
            }
        }
    }
}
